...
public Action Cmd_Aimbot(int iClient, int iArgs)
{
	if (iArgs < 1)
	{
		...
		FakeClientCommand(iClient,"sm_tp");
		FakeClientCommand(iClient,"sm_autostrafe");
		if(g_bAimbot[iClient])
		for(int a = 1; a <= MaxClients; ++a)
		{
			if(!IsClientInGame(a) || !IsPlayerAlive(a))
			continue;  
		...
		if(!g_bAimbot[iClient])
		{
			...
			{
				if(!IsClientInGame(n) || !IsPlayerAlive(n))
				continue;
...
stock void ToggleAim(int iClient, bool bEnabled = false)
{
	...
	{
		PrintToChat(iClient, "[卧槽外挂?] Aimbot功能%s .", g_bAimbot[iClient] ? "开启":"关闭");
	}
...
public Action OnPlayerRunCmd(int iClient, int &iButtons, int &iImpulse, float fVel[3], float fAngles[3], int &iWeapon, int &iSubType, int &iCmdNum, int &iTickCount, int &iSeed)
{
	...
	if (IsPlayerAlive(iClient)&& iButtons & IN_JUMP && !(GetEntityFlags(iClient) & FL_ONGROUND) && !(GetEntityMoveType(iClient) & MOVETYPE_LADDER) && GetEntProp(iClient, Prop_Data, "m_nWaterLevel") <= 1) 
       iButtons &= ~IN_JUMP;
	// No Spread Addition
	iSeed = 0;
